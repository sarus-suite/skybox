...
pub(crate) fn podman_stop(
    ...
    let stop_stdout = String::from_utf8(stop.output.stdout)?;
    let stop_stderr = String::from_utf8(stop.output.stderr)?;

    let mut msg_cmd = format!("CMD: {}", stop_command);
    ...
    */
    ...
    if rm_status == "" {
        ...
        let msg = plugin_string(format!("container {} status is: {}", name, rm_status).as_str());
        spank_log_info!("{msg}");
    }
